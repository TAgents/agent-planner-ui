    ...
    <div className="mx-auto max-w-[1180px] px-6 py-20 sm:px-9 md:py-24">
      ...
      <div className="mt-9 grid grid-cols-1 gap-[18px] md:grid-cols-3">
        {CASES.map((c) => (
          <div
            key={c.tag}
            className="flex min-h-[240px] flex-col gap-3.5 rounded-[4px] border border-border bg-surface p-6"
          >
            <span className="font-mono text-[9.5px] font-semibold uppercase tracking-[0.18em] text-amber">
              {c.tag}
            </span>
            <div className="font-display text-[21px] font-semibold leading-tight text-text">
              {c.title}
            </div>
            ...
            <div className="mt-auto flex flex-wrap gap-1.5">
              {c.examples.map((e) => (
                <span
                  key={e}
                  className="border border-border bg-surface-hi px-2 py-[3px] font-mono text-[10px] tracking-[0.02em] text-text-sec"
                >
                  {e}
                </span>
              ))}
